    ...
    return {
      ...
      setup(aibitat) {
        ...
        aibitat.function({
          ...
          name: this.name,
          description:
            "Save content to a file that will be automatically downloaded to the user's browser. DO NOT use placeholders for the file content! You MUST provide the full, final text, code, or markdown to be saved. If the user asks for a PDF, you CANNOT generate binary PDFs natively. Instead, you MUST generate a beautifully formatted HTML file (with .html extension) that embeds any requested charts using Chart.js via CDN, and includes a script to call window.print() on load. CRITICAL RULES: 1) When the user asks for a chart, graph, visualization, or downloadable file, you MUST call this tool IMMEDIATELY in the same response — do NOT just describe what you would generate and wait for confirmation. 2) When using this tool, DO NOT output the raw code or HTML in your regular chat response. Your chat response should only be a brief message confirming the file was generated. 3) You can call this tool multiple times in one turn to generate multiple files (e.g., separate charts for different data sets).",
          examples: [
            {
